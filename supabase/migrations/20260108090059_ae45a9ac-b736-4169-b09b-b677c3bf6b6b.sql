...
CREATE POLICY "Allow public read access to pods"
...
CREATE INDEX idx_pods_namespace ON public.pods(namespace);
CREATE INDEX idx_pods_status ON public.pods(status);
CREATE INDEX idx_containers_pod_id ON public.containers(pod_id);
CREATE INDEX idx_logs_container_id ON public.logs(container_id);
CREATE INDEX idx_logs_timestamp ON public.logs(timestamp DESC);

-- Enable realtime for monitoring updates
